...
public struct CoreImageFilter {
    ...
    public let parameters: [ParameterKey: Any]

    var parametersDictionaryValue: [String: Any] {
...
extension UIImage {
    public func applyingFilter(_ filters: CoreImageFilter...) -> UIImage {
        applyingFilters(filters)
    }

    public func applyingFilters(_ filters: [CoreImageFilter]) -> UIImage {
        if let input = CIImage(image: self) {
            let output = input.applyingFilters(filters)
...
extension CIImage {
    func applyingFilters(_ filters: [CoreImageFilter]) -> CIImage {
        var output = self
